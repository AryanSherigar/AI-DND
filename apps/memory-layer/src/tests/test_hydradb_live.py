...
class HydraDbLiveTests(unittest.TestCase):
    def test_graph_write_and_causal_read(self) -> None:
        ...
        plan = GraphWritePlan(
            ...
            (
                GraphNode(
                    ...
                    "Session",
                    f"session-{seed}",
                    {"context_id": context, "session_id": "live"},
                ),
                GraphNode(
                    seed + 1,
                    "Turn",
                    f"turn-{seed}",
                    {"context_id": context, "source_chunk_id": f"chunk-{seed}"},
                ),
            ),
            (
                GraphRelationship(
        ...
        rows = transport.read(
            "MATCH (s:Session {id: $id})-[:HAS_TURN]->(t:Turn) RETURN t.source_chunk_id AS chunk_id",
            {"id": seed},
            bookmarks[-1] if bookmarks else None,
        )
