...
from context_memory.ingestion.fakes import DeterministicEmbedder, InMemoryGraphIdAllocator, InMemoryGraphManifestStore
...
from context_memory.persistence.postgres import PostgresChunkStore, PostgresEmbeddingStore, PostgresSearchIndexStore
...
class FactProjectionCrossContextTests(unittest.TestCase):
    ...
    def _writer(self):
        return FactProjectionWriter(
            DeterministicEmbedder(), PostgresEmbeddingStore(self.pool),
            PostgresSearchIndexStore(self.pool), PostgresChunkStore(self.pool),
        )
    ...
    def test_direct_authored_fact_is_found_by_candidate_seeder(self) -> None:
        ...
        write_entity(context_id, DirectEntityInput(canonical_name="Sukuna", entity_type="character"), allocator, graph_writer)

        fact_id = write_fact(
            context_id,
            DirectFactInput(predicate="status", subject_canonical_name="Sukuna", object_literal="alive"),
            allocator, graph_writer, fact_projector=projector,
        )

        seeded = self._seeder().seed(context_id, "Sukuna status alive", _NO_REWRITE, top_k=5)
        self.assertIn(str(fact_id), seeded)

    def test_cloning_the_same_template_fact_into_two_playthroughs_does_not_collide(self) -> None:
        source_transport = _FakeHydraReadTransport(
            ...
            fact_rows=[{
                "id": 2, "logical_key": "fact:direct:it-shared-hash", "text": "Sukuna status alive",
        ...
        clone("it-clone-template", playthrough_a, allocator, writer_a, source_transport, fact_projector=projector)
        clone("it-clone-template", playthrough_b, allocator, writer_b, source_transport, fact_projector=projector)

        new_id_a = allocator.allocate_graph_id("fact", playthrough_a, "fact:direct:it-shared-hash")
        new_id_b = allocator.allocate_graph_id("fact", playthrough_b, "fact:direct:it-shared-hash")
        self.assertNotEqual(new_id_a, new_id_b)

        seeded_a = self._seeder().seed(playthrough_a, "Sukuna status alive", _NO_REWRITE, top_k=5)
        seeded_b = self._seeder().seed(playthrough_b, "Sukuna status alive", _NO_REWRITE, top_k=5)
        ...
        with self.pool.connection() as conn:
            with conn.cursor() as cursor:
                cursor.execute(
                    "SELECT context_id, raw_text FROM fact_search_index WHERE fact_id = %s", (str(new_id_a),)
                )
