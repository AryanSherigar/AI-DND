...
HAS_KEY = bool(
    os.environ.get("FIREWORKS_API_KEY") or os.environ.get("ENTITY_RESOLUTION_API_KEY")
)


@unittest.skipUnless(
    HAS_KEY,
    "requires FIREWORKS_API_KEY (or ENTITY_RESOLUTION_API_KEY) for a real provider call",
)
class LiveEntityResolutionTests(unittest.TestCase):
    ...
    def test_resolves_alias_within_bounded_candidates(self) -> None:
        candidates = (
            EntityProfile(
                1, "context:1", "max the golden retriever", "pet", ("my dog",)
            ),
            EntityProfile(2, "context:1", "max the colleague", "person"),
        )
        selected = self.model.resolve_entity(
            context_id="context:1", surface="my dog", candidates=candidates
        )
...
@unittest.skipUnless(
    HAS_KEY,
    "requires FIREWORKS_API_KEY (or TEMPORAL_UPDATE_API_KEY) for a real provider call",
)
class LiveTemporalUpdateTests(unittest.TestCase):
    ...
    def test_real_world_move_is_a_state_change(self) -> None:
        ...
        new = FactState(
            ...
            1,
            "lives_in",
            "Max just moved to Seattle last week",
            datetime(2026, 2, 1, tzinfo=timezone.utc),
        )
        self.assertEqual(
            self.model.classify_update(new_fact=new, prior_fact=prior),
    ...
    def test_explicit_correction_is_not_a_state_change(self) -> None:
        ...
        new = FactState(
            ...
            1,
            "lives_in",
            "I got that wrong earlier -- Max has always lived in Boston, never Seattle",
