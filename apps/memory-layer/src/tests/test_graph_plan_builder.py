...
class GraphPlanBuilderTests(unittest.TestCase):
    ...
    def test_fact_node_carries_subject_and_object_literal_properties(self) -> None:
        ...
        candidate = _candidate(entities=(EntityCandidate("Max", "pet"),), subject="Max", object="the park")
        extraction = ExtractionResult("attempt-1", (candidate,), ())
        profile = EntityProfile(99, self.chunk.context_id, "Max (pet)", "pet")
        plan = self.builder.build(self.chunk, extraction, resolve=lambda cid, surface, etype: profile)
    ...
    def test_fact_node_omits_subject_and_object_when_extraction_left_them_blank(self) -> None:
        candidate = _candidate(entities=(EntityCandidate("Max", "pet"),))
        extraction = ExtractionResult("attempt-1", (candidate,), ())
        profile = EntityProfile(99, self.chunk.context_id, "Max (pet)", "pet")
        plan = self.builder.build(self.chunk, extraction, resolve=lambda cid, surface, etype: profile)
    ...
    def test_resolve_many_path_produces_the_same_plan_as_the_per_mention_path(self) -> None:
        ...
        plan_via_resolve = self.builder.build(self.chunk, extraction, resolve=lambda cid, surface, etype: profile)
        ...
        plan_via_batch = self.builder.build(self.chunk, extraction, resolve=lambda *a: None, resolve_many=resolve_many)
    ...
    def test_resolve_many_mentions_map_back_to_the_correct_fact_in_order(self) -> None:
        ...
        max_candidate = ExtractedMemoryCandidate(
            candidate_id="fact-max", text="Max likes walks", memory_type=MemoryType.SEMANTIC,
            scope_type=MemoryScope.SESSION, scope_id="session-001",
            source_span=SourceSpan("record-001", 0, 3), confidence=0.9,
            temporal=TemporalBounds(datetime(2026, 1, 10, 9, tzinfo=timezone.utc)),
            entities=(EntityCandidate("Max", "pet"),),
        ...
        rex_candidate = ExtractedMemoryCandidate(
            candidate_id="fact-rex", text="Rex likes running", memory_type=MemoryType.SEMANTIC,
            scope_type=MemoryScope.SESSION, scope_id="session-001",
            source_span=SourceSpan("record-001", 0, 3), confidence=0.9,
            temporal=TemporalBounds(datetime(2026, 1, 10, 9, tzinfo=timezone.utc)),
        ...
        plan = self.builder.build(self.chunk, extraction, resolve=lambda *a: None, resolve_many=resolve_many)

        about_edges = {r.source_id: r for r in plan.relationships if r.relationship_type == "ABOUT"}
        max_fact_id = self.allocator.allocate_graph_id("fact", "context-001", "fact:fact-max")
        rex_fact_id = self.allocator.allocate_graph_id("fact", "context-001", "fact:fact-rex")
        self.assertEqual(about_edges[max_fact_id].destination_id, 101)  # Max's fact links to Max's profile, not Rex's
    ...
    def test_no_entities_in_chunk_never_calls_resolve_many(self) -> None:
        extraction = ExtractionResult("attempt-1", (), ())
        called = []
        self.builder.build(self.chunk, extraction, resolve=lambda *a: None, resolve_many=lambda *a: called.append(a) or [])
