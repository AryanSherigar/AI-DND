...
class HydraHttpTransportTests(unittest.TestCase):
    def test_write_uses_local_query_endpoint_and_query_id(self) -> None:
        ...
        transport = HydraHttpTransport(
            "http://127.0.0.1:8443", "local-token", requester=requester
        )
        self.assertEqual(
            transport.write(
                "UNWIND $rows AS row RETURN row.id", [{"id": 1}], "stable-key"
            ),
            "bookmark-1",
        )
        method, url, headers, body = calls[0]
        self.assertEqual(
            (method, url), ("POST", "http://127.0.0.1:8443/v1/graphs/default/query")
        )
        ...
        self.assertEqual(
            ...
            {
                "cell_id": "cell-0",
                "parameters": {"rows": [{"id": 1}]},
                "query": "UNWIND $rows AS row RETURN row.id",
                "query_id": "stable-key",
            },
        )
    ...
    def test_read_decodes_typed_values_and_forwards_bookmark(self) -> None:
        ...
        def requester(
            _: str, __: str, ___: dict[str, str], body: bytes
        ) -> dict[str, object]:
        ...
        self.assertEqual(
            transport.read("MATCH", {"id": 1}, "bookmark-1"),
            [{"name": "Max", "count": 2}],
        )
