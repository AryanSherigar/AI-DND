...
class _BatchRecordingModel:
    ...
    def classify_updates(self, *, new_fact, prior_facts):
        ...
        return {i: self._relations.get(p.fact_id, TemporalRelation.UNRESOLVED) for i, p in enumerate(prior_facts)}

class BatchedTemporalUpdateTests(unittest.TestCase):
    """§8: one call for N priors instead of N calls."""

    def fact(self, fact_id: str, day: int, *, subject: int = 7, predicate: str = "breed", text: str | None = None) -> FactState:
        return FactState(fact_id, subject, predicate, text or f"fact {fact_id}",
    ...
    def test_many_priors_become_one_batched_call(self) -> None:
        ...
        decisions = TemporalUpdateClassifier(model).classify_many(new_fact=new, prior_facts=priors)
    ...
    def test_missing_index_in_response_degrades_only_that_prior(self) -> None:
        ...
        decisions = TemporalUpdateClassifier(_PartialModel()).classify_many(new_fact=new, prior_facts=priors)
    ...
    def test_batch_disabled_falls_back_to_pairwise(self) -> None:
        ...
        model = _BatchRecordingModel()
        TemporalUpdateClassifier(model, batch_enabled=False).classify_many(new_fact=new, prior_facts=priors)
        self.assertEqual(model.batch_calls, [])
...
class SimilarityPrefilterTests(unittest.TestCase):
    ...
    def fact(self, fact_id: str, day: int, text: str, *, subject: int = 7, predicate: str = "p") -> FactState:
        return FactState(fact_id, subject, predicate, text,
                         datetime(2026, 1, 1, tzinfo=timezone.utc) + timedelta(days=day))
    ...
    def test_dissimilar_priors_never_reach_the_model(self) -> None:
        ...
        priors = [self.fact("rel", 1, "related prior"), self.fact("unrel", 2, "unrelated prior")]
    ...
    def test_all_dissimilar_means_zero_model_calls(self) -> None:
        new = self.fact("new", 30, "new fact")
        priors = [self.fact("u1", 1, "unrelated prior"), self.fact("u2", 2, "unrelated prior")]
        model = _BatchRecordingModel()
        classifier = TemporalUpdateClassifier(
            model, embedder=_StubEmbedder(self.vectors), similarity_threshold=0.15
        )
        decisions = classifier.classify_many(new_fact=new, prior_facts=priors)
        self.assertEqual(model.batch_calls, [])
        self.assertEqual([d.relation for d in decisions], [TemporalRelation.NO_UPDATE] * 2)
