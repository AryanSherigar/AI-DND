...
class LongMemEvalAdapterTests(unittest.TestCase):
    def input_instance(self) -> dict[str, object]:
        return json.loads((FIXTURES / "longmemeval_adapter_input_v1.json").read_text())[
            0
        ]

    def test_adapter_matches_canonical_fixture_and_sorts_sessions(self) -> None:
        batch = adapt_longmemeval_instance(
            self.input_instance(), "fixture-longmemeval-v1-run-001"
        )
        expected_payload = json.loads(
            (
                FIXTURES / "longmemeval_adapter_expected_context_batch_v1.json"
            ).read_text()
        )
    ...
    def test_custom_minute_timestamp_parses_to_ordering_basis(self) -> None:
        normalized = parse_longmemeval_timestamp(
            "2021/05/03 (Mon) 09:02", "fixture.date"
        )
    ...
    def test_evaluation_labels_are_not_canonical_metadata(self) -> None:
        ...
        self.assertTrue(
            all(not forbidden.intersection(record.metadata) for record in batch.records)
        )
    ...
    def test_cli_dry_run_does_not_require_database(self) -> None:
        ...
        self.assertEqual(
            completed.stdout.strip(), "validated_batches=1 validated_records=2"
        )
