...
class OrchestratorTests(unittest.TestCase):
    ...
    def test_extraction_provider_failure_is_retryable_not_a_silent_completion(self) -> None:
        ...
        batch, record = self.batch_and_record()
        extraction_service = ExtractionService(_FlakyExtractor(), InMemoryExtractionStore())
        transport = RecordingGraphTransport()
        orchestrator = IngestionOrchestrator(
            chunk_store=self.chunk_store, job_store=self.job_store, extraction_service=extraction_service,
            graph_plan_builder=GraphPlanBuilder(self.allocator), graph_writer=GraphWriter(self.manifest_store, transport),
    ...
    def test_verification_fails_the_chunk_when_embedding_row_is_missing(self) -> None:
        ...
        batch, record = self.batch_and_record()
        extractor = DeterministicExtractor({
            "record-001": (ExtractionDraft("fact-001", "Max likes walks", 0, 3, 0.9, entities=(EntityCandidate("Max", "pet"),)),)
        })
        extraction_service = ExtractionService(extractor, InMemoryExtractionStore())
        transport = RecordingGraphTransport()
        orchestrator = IngestionOrchestrator(
            chunk_store=self.chunk_store, job_store=self.job_store, extraction_service=extraction_service,
            graph_plan_builder=GraphPlanBuilder(self.allocator), graph_writer=GraphWriter(self.manifest_store, transport),
            resolve_entity=lambda cid, surface, etype: self.entity_registry.resolve(context_id=cid, surface=surface, entity_type=etype).entity,
            embedder=DeterministicEmbedder(), embedding_store=DroppingEmbeddingStore(),
            search_index_store=self.search_index_store,
        )
        result = orchestrator.run_record(batch, record)
        self.assertEqual(result.state, IngestionJobState.RETRYABLE_FAILED)
    ...
    def test_verification_fails_the_chunk_when_search_index_row_is_missing(self) -> None:
    ...
    def test_safe_transition_logs_loudly_instead_of_swallowing_silently(self) -> None:
        ...
        with self.assertLogs("context_memory.ingestion.orchestrator", level="ERROR") as logs:
            orchestrator._safe_transition("chunk-x", IngestionJobState.RETRYABLE_FAILED, "original failure")
        self.assertTrue(any("RECONCILIATION REQUIRED" in message for message in logs.output))
...
class GroupedRunBatchTests(unittest.TestCase):
    ...
    def make_batch(self, n: int, session_id: str = "session-001") -> tuple[ContextBatch, list[ContextRecord]]:
        records = [
            ContextRecord(
                record_id=f"record-{i:03d}", session_id=session_id,
                occurred_at=datetime(2026, 1, 10, 9, i, tzinfo=timezone.utc), content=f"Fact number {i} about the user",
            )
            for i in range(n)
        ]
        batch = ContextBatch("ingestion-001", "context-001", SourceDescriptor("fixture", "fixture-001"), tuple(records))
        return batch, records

    def orchestrator(self, transport, write_batch_size: int, candidates_by_record: dict | None = None) -> IngestionOrchestrator:
        ...
        return IngestionOrchestrator(
            ...
            graph_plan_builder=plan_builder, graph_writer=graph_writer, resolve_entity=resolve_entity,
            embedder=DeterministicEmbedder(), embedding_store=self.embedding_store,
            search_index_store=self.search_index_store, write_batch_size=write_batch_size,
        )

    def candidates_for(self, records: list[ContextRecord]) -> dict:
        return {
            record.record_id: (
                ExtractionDraft(f"fact-{record.record_id}", f"Fact text for {record.record_id}", 0, 3, 0.9, entities=(EntityCandidate("Max", "pet"),)),
            )
    ...
    def test_grouped_batch_completes_all_chunks_in_order(self) -> None:
        ...
        transport = RecordingGraphTransport()
        orchestrator = self.orchestrator(transport, write_batch_size=10, candidates_by_record=self.candidates_for(records))
        result = orchestrator.run_batch(batch)
        self.assertEqual(result.completed_count, 4)
        self.assertEqual([r.state for r in result.results], [IngestionJobState.COMPLETED] * 4)
        ...
        expected_chunk_ids = [chunk_from_record(batch, record).chunk_id for record in records]
    ...
    def test_grouped_batch_uses_fewer_write_calls_than_per_chunk(self) -> None:
        ...
        self.orchestrator(transport_batched, write_batch_size=10, candidates_by_record=candidates).run_batch(batch)
        ...
        self.orchestrator(transport_unbatched, write_batch_size=1, candidates_by_record=candidates2).run_batch(batch2)
    ...
    def test_grouped_batch_respects_group_boundary(self) -> None:
        ...
        self.orchestrator(transport_one_group, write_batch_size=10, candidates_by_record=candidates).run_batch(batch)

        self.setUp()
        batch2, records2 = self.make_batch(4)
        candidates2 = self.candidates_for(records2)
        transport_two_groups = RecordingGraphTransport()
        self.orchestrator(transport_two_groups, write_batch_size=2, candidates_by_record=candidates2).run_batch(batch2)

        self.assertLess(len(transport_one_group.writes), len(transport_two_groups.writes))

    def test_already_completed_chunk_is_excluded_from_batch_but_group_mates_still_process(self) -> None:
        batch, records = self.make_batch(3)
        candidates = self.candidates_for(records)
        transport = RecordingGraphTransport()
        orchestrator = self.orchestrator(transport, write_batch_size=10, candidates_by_record=candidates)
        ...
        self.assertEqual(result.results[0].accepted_fact_count, 0)  # short-circuited, not re-extracted
        self.assertEqual(result.results[1].accepted_fact_count, 1)
        self.assertEqual(result.results[2].accepted_fact_count, 1)

    def test_batched_graph_write_failure_marks_whole_group_retryable_not_lost(self) -> None:
        ...
        batch, records = self.make_batch(3)
        candidates = self.candidates_for(records)
        orchestrator = self.orchestrator(_AlwaysFailsTransport(), write_batch_size=10, candidates_by_record=candidates)
        result = orchestrator.run_batch(batch)
        # All three failed together (one batched write covering the group), none silently dropped.
        self.assertEqual(len(result.results), 3)
        self.assertTrue(all(r.state == IngestionJobState.RETRYABLE_FAILED for r in result.results))
