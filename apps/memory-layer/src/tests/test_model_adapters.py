...
class _FakeModels:
    ...
    def generate_content(self, *, model, contents, config) -> types.GenerateContentResponse:
...
class BatchedEntityResolutionModelTests(unittest.TestCase):
    ...
    def test_batch_prompt_numbers_mentions_and_max_tokens_scales_with_count(self) -> None:
        ...
        expected = config.entity_resolution_max_tokens + config.entity_resolution_batch_tokens_per_mention * 2
...
class LLMExtractorTests(unittest.TestCase):
    ...
    def test_extract_facts_carries_subject_and_object_as_real_triple_components(self) -> None:
        ...
        self.assertEqual(drafts[0].text, "The dog is currently waiting near the western gate.")
    ...
    def test_extract_malformed_response_raises_instead_of_silently_returning_empty(self) -> None:
    ...
    def test_extraction_max_tokens_tiers_by_content_length(self) -> None:
        ...
        def make_record(content: str) -> ContextRecord:
            return ContextRecord(
                record_id="rec:tier", session_id="session:001", actor_role="user",
                occurred_at=datetime.now(timezone.utc), content_type="text/plain", content=content,
            )
        ...
        self.assertLess(config.extractor_max_tokens_tier_short, config.extractor_max_tokens_tier_medium)
        self.assertLess(config.extractor_max_tokens_tier_medium, config.extractor_max_tokens_tier_long)
        self.assertLess(config.extractor_max_tokens_tier_long, config.extractor_max_tokens_tier_xlong)

    def test_extraction_max_tokens_for_boundaries_are_inclusive(self) -> None:
        from context_memory.core.config import Config

        config = Config()
        self.assertEqual(config.extraction_max_tokens_for(config.extractor_max_tokens_tier_short_chars), config.extractor_max_tokens_tier_short)
        self.assertEqual(config.extraction_max_tokens_for(config.extractor_max_tokens_tier_short_chars + 1), config.extractor_max_tokens_tier_medium)
        self.assertEqual(config.extraction_max_tokens_for(config.extractor_max_tokens_tier_medium_chars), config.extractor_max_tokens_tier_medium)
        self.assertEqual(config.extraction_max_tokens_for(config.extractor_max_tokens_tier_medium_chars + 1), config.extractor_max_tokens_tier_long)
        self.assertEqual(config.extraction_max_tokens_for(config.extractor_max_tokens_tier_long_chars), config.extractor_max_tokens_tier_long)
        self.assertEqual(config.extraction_max_tokens_for(config.extractor_max_tokens_tier_long_chars + 1), config.extractor_max_tokens_tier_xlong)
...
class LLMExtractorBatchTests(unittest.TestCase):
    ...
    def test_happy_path_maps_facts_back_to_the_right_record(self) -> None:
        response_json = json.dumps({
            "turns": [
                {"turn_index": 0, "facts": [
                    {"text": "User owns a dog named Max", "predicate_key": "pet_name",
                     "entities": ["Max"], "confidence": 0.98, "exact_quote": "dog named Max"},
                ]},
                {"turn_index": 1, "facts": [
                    {"text": "User works as a teacher", "predicate_key": "occupation",
                     "entities": ["teacher"], "confidence": 0.95, "exact_quote": "I'm a teacher"},
                ]},
            ]
        })
        extractor = LLMExtractor(_client(response_json))
        ...
        self.assertEqual(
            records[0].content[results["rec:a"][0].source_start:results["rec:a"][0].source_end],
            "dog named Max",
        )
        self.assertEqual(
            records[1].content[results["rec:b"][0].source_start:results["rec:b"][0].source_end],
            "I'm a teacher",
        )
    ...
    def test_missing_turn_index_degrades_that_one_record_only(self) -> None:
        ...
        response_json = json.dumps({
            "turns": [
                {"turn_index": 0, "facts": [
                    {"text": "User owns a dog named Max", "entities": ["Max"], "exact_quote": "dog named Max"},
                ]},
            ]
        })
        extractor = LLMExtractor(_client(response_json))
    ...
    def test_malformed_batch_response_raises_instead_of_returning_empty_for_every_record(self) -> None:
    ...
    def test_whitespace_only_records_are_skipped_but_still_present_in_the_result(self) -> None:
    ...
    def test_batch_prompt_numbers_turns_and_max_tokens_sums_per_turn_tiers(self) -> None:
        from context_memory.core.config import Config
        ...
        expected = config.extractor_max_tokens_tier_short + config.extractor_max_tokens_tier_medium
        self.assertEqual(call["config"].max_output_tokens, expected)
