...
from context_memory.ingestion.fakes import InMemoryGraphIdAllocator, InMemoryGraphManifestStore, RecordingGraphTransport
...
class WriteEntityTests(unittest.TestCase):
    def test_writes_one_entity_node_with_creator_fields(self):
        ...
        graph_id = write_entity(
            ...
            DirectEntityInput(canonical_name="Sukuna", entity_type="character", description="A cursed spirit"),
            allocator, writer,
        )
    ...
    def test_aliases_are_flattened_to_a_scalar_property(self):
        ...
        write_entity(
            "ctx-1",
            DirectEntityInput(canonical_name="Sukuna", entity_type="character", aliases=("King of Curses", "Ryomen")),
            allocator, writer,
        )
...
class WriteFactTests(unittest.TestCase):
    def test_writes_fact_node_and_about_edge_to_subject(self):
        ...
        fact_write = next(w for w in transport.writes if "Fact" in w[0] and "Entity" not in w[0])
    ...
    def test_object_entity_reference_gets_a_relates_to_edge(self):
        allocator, writer, transport = _writer()
        write_entity("ctx-1", DirectEntityInput(canonical_name="Sukuna", entity_type="character"), allocator, writer)
        write_entity("ctx-1", DirectEntityInput(canonical_name="Jujutsu High", entity_type="faction"), allocator, writer)

        write_fact(
            "ctx-1",
            DirectFactInput(predicate="member_of", subject_canonical_name="Sukuna", object_canonical_name="Jujutsu High"),
            allocator, writer,
        )
    ...
    def test_object_literal_stored_without_a_relates_to_edge(self):
        ...
        write_fact(
            "ctx-1",
            DirectFactInput(predicate="has_power_level", subject_canonical_name="Sukuna", object_literal="9999"),
            allocator, writer,
        )
    ...
    def test_identical_triple_written_twice_is_idempotent(self):
        allocator, writer, _transport = _writer()
        write_entity("ctx-1", DirectEntityInput(canonical_name="Sukuna", entity_type="character"), allocator, writer)
        fact = DirectFactInput(predicate="is_strongest", subject_canonical_name="Sukuna", object_literal="true")
    ...
    def test_different_playthrough_contexts_get_independent_graph_ids(self):
        ...
        allocator, writer, _transport = _writer()
        write_entity("scenario-template::s1", DirectEntityInput(canonical_name="Sukuna", entity_type="character"), allocator, writer)
        write_entity("playthrough-1", DirectEntityInput(canonical_name="Sukuna", entity_type="character"), allocator, writer)
        fact = DirectFactInput(predicate="is_strongest", subject_canonical_name="Sukuna", object_literal="true")
...
class HiddenFactTests(unittest.TestCase):
    ...
    def test_hidden_true_persists_metadata(self):
        ...
        fact_id = write_fact(
            "ctx-1",
            DirectFactInput(predicate="knows_secret", subject_canonical_name="Sukuna", object_literal="true", hidden=True),
            allocator, writer, metadata_store,
        )

        self.assertEqual(metadata_store.puts, [("ctx-1", fact_id, None, None, None, True)])
...
class SupersededFactIdTests(unittest.TestCase):
    ...
    def test_registers_its_own_external_fact_id(self):
        ...
        fact_id = write_fact(
            "ctx-1",
            DirectFactInput(
                predicate="is_alive", subject_canonical_name="Sukuna", object_literal="true",
                external_fact_id="core-api-fact-1",
            ),
            allocator, writer, external_fact_id_store=external_store,
        )

        resolved = external_store.get("ctx-1", "core-api-fact-1")
        self.assertEqual(resolved[0], fact_id)

    def test_superseded_fact_id_creates_a_supersedes_edge_and_closes_the_prior_fact(self):
        allocator, writer, transport = _writer()
        write_entity("ctx-1", DirectEntityInput(canonical_name="Sukuna", entity_type="character"), allocator, writer)
        external_store = FakeExternalFactIdStore()
        ...
        new_fact_id = write_fact(
            "ctx-1",
            DirectFactInput(
                predicate="is_alive", subject_canonical_name="Sukuna", object_literal="false",
                external_fact_id="core-api-fact-2", superseded_fact_id="core-api-fact-1",
            ),
            allocator, writer, external_fact_id_store=external_store,
        )
        ...
        closing_rows = [
            row for _, rows, _ in transport.writes for row in rows
    ...
    def test_unresolvable_superseded_fact_id_raises(self):
        ...
        with self.assertRaises(ValueError):
            write_fact(
                ...
                allocator, writer, external_fact_id_store=FakeExternalFactIdStore(),
            )
