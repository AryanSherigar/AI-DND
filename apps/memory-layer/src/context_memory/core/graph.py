    ...
    for key, item in value.items():
        if not isinstance(key, str) or not key.replace("_", "").isalnum():
            raise ContractValidationError(
                field, "property keys must be identifier-safe"
            )
...
class GraphRelationship:
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.graph_id, int) or self.graph_id < 0:
            raise ContractValidationError(
                "graph_relationship.graph_id", "must be non-negative"
            )
        if self.relationship_type not in RELATIONSHIP_TYPES:
            raise ContractValidationError(
                "graph_relationship.relationship_type", "must be supported"
            )
        _required(self.logical_key, "graph_relationship.logical_key")
        if not isinstance(self.source_id, int) or self.source_id < 0:
            raise ContractValidationError(
                "graph_relationship.source_id", "must be non-negative"
            )
        if not isinstance(self.destination_id, int) or self.destination_id < 0:
            raise ContractValidationError(
                "graph_relationship.destination_id", "must be non-negative"
            )
        if (
            self.source_label not in NODE_LABELS
            or self.destination_label not in NODE_LABELS
        ):
            raise ContractValidationError(
                "graph_relationship", "endpoint labels must be supported"
            )
...
class GraphWritePlan:
    ...
    def __post_init__(self) -> None:
        ...
        if len(ids) != len(set(ids)):
            raise ContractValidationError(
                "graph_plan", "graph IDs must be globally unique"
            )
        for record in records:
            if record.properties.get("context_id") != self.context_id:
                raise ContractValidationError(
                    "graph_plan", "records must use plan context_id"
                )
