...
def chunk_id_for(context_id: str, record_id: str) -> str:
    ...
    logical_key = f"{context_id}\x00{record_id}".encode()
...
def validate_candidate(
    candidate: ExtractedMemoryCandidate, records: Iterable[ContextRecord]
) -> None:
    ...
    if source is None:
        raise ContractValidationError(
            "candidate.source_record_id", "does not identify a record in this batch"
        )
    ...
    if not 0 <= span.source_start < span.source_end <= len(source.content):
        raise ContractValidationError(
            "candidate.source_span", "must be a non-empty Unicode code-point span"
        )
    if candidate.temporal.observed_at != source.occurred_at:
        raise ContractValidationError(
            "candidate.observed_at", "must equal source record occurred_at"
        )
    valid_from = candidate.temporal.valid_from
    valid_to = candidate.temporal.valid_to
    if valid_from is not None and valid_to is not None and valid_from > valid_to:
        raise ContractValidationError(
            "candidate.valid_from", "must not be later than valid_to"
        )
