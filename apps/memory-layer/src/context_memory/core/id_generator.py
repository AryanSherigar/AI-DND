...
class IdGenerator:
    ...
    def _validate_str(value: str, field_name: str) -> str:
        if not isinstance(value, str):
            raise InvalidSemanticInputError(
                f"Field '{field_name}' must be a string, got {type(value).__name__}"
            )
        trimmed = value.strip()
        if not trimmed:
            raise InvalidSemanticInputError(
                f"Field '{field_name}' cannot be empty or blank"
            )
        return trimmed
    ...
    def turn_id(
        self, haystack_id: str, session_id: str, turn_index: int, role: str
    ) -> str:
        clean_hid = self._validate_str(haystack_id, "haystack_id")
        clean_sid = self._validate_str(session_id, "session_id")
    ...
    def alias_id(
        self, haystack_id: str, canonical_alias: str, parent_entity_canonical: str
    ) -> str:
        clean_hid = self._validate_str(haystack_id, "haystack_id")
        clean_alias = self._validate_str(canonical_alias, "canonical_alias").lower()
        clean_parent = self._validate_str(
            parent_entity_canonical, "parent_entity_canonical"
        ).lower()
