...
falls back to calling `resolve` once per mention, identical to before this existed."""
...
class GraphPlanBuilder:
    ...
    def build(
        ...
        # calls across every mention in the chunk (not the candidate
        # generation or the registry writes, both of which stay serial
        # inside `resolve_many` itself). Falls back to resolving inline,
        # one mention at a time via `resolve`, when it isn't supplied --
        # identical to this method's behavior before `resolve_many` existed.
        # See `EntityRegistry.resolve_many`'s docstring for the accepted
        # tradeoff and docs/fixes_and_evaluation_findings.md §4.7 for why.
        preresolved: list[EntityProfile | None] | None = None
        if resolve_many is not None:
            all_mentions = [
                (entity_candidate.surface, entity_candidate.entity_type or "other")
                for candidate in extraction.accepted
                for entity_candidate in candidate.entities
            ]
            preresolved = resolve_many(context_id, all_mentions) if all_mentions else []
        mention_cursor = 0
    ...
    def _fact_node(self, chunk: Chunk, candidate: ExtractedMemoryCandidate, created_at_dt: datetime) -> GraphNode:
        ...
        properties = _scalar_properties(
            ...
            session_id=chunk.session_id or "unknown",
            # §4/§5 fix: the turn a fact was extracted from, straight from
            # the chunk's own metadata (engine.submit_batch sets both
            # `turn_number`/`turn_index` to the same value) -- lets
            # `as_of_turn` retrieval filter on real turn ordering instead of
            # the synthetic/shared `observed_at` timestamp alone.
            turn_number=(chunk.metadata or {}).get("turn_number"),
            ...
            predicate_key=candidate.predicate_key,
            # §9 fix: normalized triple components straight from extraction,
            # independent of whether entity resolution below links `subject`
            # to a graph Entity. `object_literal` matches the property name
            # `direct_authoring.write_fact` already uses for a scalar object,
            # so retrieval reads one convention regardless of fact source.
            subject=candidate.subject,
            object_literal=candidate.object,
