...
class TemporalUpdateClassifier:
    ...
    def __init__(
        self, model: TemporalUpdateModel, batch_enabled: bool = True,
        embedder: object | None = None, similarity_threshold: float = 0.0,
    ) -> None:
        self._model = model
    ...
    def _embed_all(self, texts: list[str]) -> dict[str, tuple[float, ...]]:
        ...
        if missing:
            ...
            try:
                ...
                logger.warning("temporal-update prefilter embedding failed, skipping filter: %s", error)
                return {}
        return {t: self._embed_cache[t] for t in texts if t in self._embed_cache}

    def _prefilter(self, new_fact: FactState, candidates: list[tuple[int, FactState]]) -> tuple[list[tuple[int, FactState]], list[int]]:
        ...
        """
        if self._embedder is None or self._similarity_threshold <= 0.0 or not candidates:
            return candidates, []
        ...
        for index, prior in candidates:
            ...
            elif sum(a * b for a, b in zip(new_vector, prior_vector)) >= self._similarity_threshold:
                kept.append((index, prior))
            else:
                dropped.append(index)
        return kept, dropped

    def _gate(self, new_fact: FactState, prior_fact: FactState) -> TemporalUpdateDecision | None:
    ...
    def classify_many(self, *, new_fact: FactState, prior_facts: Sequence[FactState]) -> list[TemporalUpdateDecision]:
        ...
        for index in prefiltered:
            decisions[index] = TemporalUpdateDecision(TemporalRelation.NO_UPDATE, "below similarity threshold")
        if not to_ask:
            return [d if d is not None else TemporalUpdateDecision(TemporalRelation.UNRESOLVED, "no decision") for d in decisions]

        batch = self._batch_enabled and isinstance(self._model, BatchTemporalUpdateModel)
        if batch:
            relations = self._model.classify_updates(new_fact=new_fact, prior_facts=[p for _, p in to_ask])
            for slot, (i, _) in enumerate(to_ask):
                decisions[i] = self._decide(new_fact, relations.get(slot, TemporalRelation.UNRESOLVED))
        else:
    ...
    def classify(self, *, new_fact: FactState, prior_fact: FactState) -> TemporalUpdateDecision:
        ...
        if gated is not None:
            return gated
        return self._decide(new_fact, self._model.classify_update(new_fact=new_fact, prior_fact=prior_fact))

    def _decide(self, new_fact: FactState, relation: TemporalRelation) -> TemporalUpdateDecision:
