...
class EndToEndManualTestRunner:
    def __init__(self, live: bool = False, api_key: str | None = None, model_name: str | None = None) -> None:
        ...
        if self.live:
            ...
            # Vertex AI migration: no more base_url/OpenAI-compatible
            # endpoints -- `google-genai`'s API-key auth mode resolves its
            # own Vertex endpoint internally.
            key = (
                api_key or os.getenv("GOOGLE_CLOUD_API_KEY")
                or os.getenv("Gemini_agentic_platform_api_key") or os.getenv("LLM_API_KEY")
            )
            if not key:
                raise ValueError(
                    "Running in --live mode requires GOOGLE_CLOUD_API_KEY "
                    "(or Gemini_agentic_platform_api_key/LLM_API_KEY)."
                )
            model = model_name or os.getenv("EXTRACTOR_MODEL") or "gemini-3.8-flash"
            print(f"Using LIVE LLM (Vertex AI): model={model}")
            self.llm = LLMClient(api_key=key, model_name=model)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--live", action="store_true", help="Run with real live LLM provider (Vertex AI)")
    parser.add_argument("--api-key", default=None, help="Vertex AI (google-genai) API key")
