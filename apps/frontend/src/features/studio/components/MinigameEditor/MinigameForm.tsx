...
const tierRangesOverlap = (ranges: TieredOutcomeRange[]): boolean => {
  const ordered = [...ranges].sort((left, right) => left.min_score - right.min_score);
  return ordered.some(
    (range, index) => index > 0 && range.min_score <= ordered[index - 1].max_score,
  );
};
...
const buildInitialState = (
  ...
  dodgeDifficulty: minigame?.dodge_config?.difficulty ?? DEFAULT_DODGE_DIFFICULTY,
  dodgeConfig: {
    ...DEFAULT_DODGE_CONFIG,
...
const canSubmitForm = (formState: MinigameFormState): boolean => {
  ...
  if (formState.outcomeMode === "tiered" && tierRangesOverlap(formState.tieredOutcomes)) {
    return false;
  }
  return true;
};
...
export const MinigameForm: React.FC<MinigameFormProps> = ({
  ...
  const togglePattern = (pattern: NonNullable<DodgeConfigShape["enabled_patterns"]>[number]): void => {
  ...
  const handleAudioUpload = (event: React.ChangeEvent<HTMLInputElement>): void => {
    ...
    uploadScenarioAudio.mutate(file, {
      ...
      onError: () => setAudioUploadError("Audio upload failed — please try again."),
    });
  };
        ...
        <>
          ...
          <div className="mt-3 space-y-4">
            <p className="text-xs text-content-faint">
              Hitboxes are fixed for fairness. Longer encounters with low health or
              short invulnerability can be difficult even at a low preset.
            </p>
            {(formState.dodgeConfig.duration_seconds ?? 15) > 60 &&
              (formState.dodgeConfig.health ?? 3) < 3 && (
                <p className="text-xs text-warning">
                  Fairness warning: a long encounter with fewer than 3 health is likely unforgiving.
                </p>
              )}
            <div className="grid grid-cols-3 gap-2">
              <Input aria-label="Duration seconds" type="number" min={10} max={120}
                value={formState.dodgeConfig.duration_seconds ?? 15}
                onChange={(e) => updateDodgeConfig({ duration_seconds: Number(e.target.value) })} />
              <Input aria-label="Health" type="number" min={1} max={10}
                value={formState.dodgeConfig.health ?? 3}
                onChange={(e) => updateDodgeConfig({ health: Number(e.target.value) })} />
              <Input aria-label="Invulnerability milliseconds" type="number" min={250} max={3000}
                value={formState.dodgeConfig.invulnerability_ms ?? 1000}
                onChange={(e) => updateDodgeConfig({ invulnerability_ms: Number(e.target.value) })} />
            </div>
            <div>
              <p className="mb-1 text-sm text-content-muted">Enabled patterns (run in this order)</p>
              <div className="flex gap-3">
                {(["rain", "ring", "beam", "homing"] as const).map((pattern) => (
                  <label key={pattern} className="text-sm text-content-muted">
                    <input type="checkbox" checked={formState.dodgeConfig.enabled_patterns?.includes(pattern) ?? false}
                      disabled={(formState.dodgeConfig.enabled_patterns?.length ?? 0) === 1 && formState.dodgeConfig.enabled_patterns?.includes(pattern)}
                      onChange={() => togglePattern(pattern)} /> {pattern}
                  </label>
                ))}
              </div>
              <p className="mt-1 text-xs text-content-faint">Uncheck patterns to disable them; checked patterns retain this curated order. At least one pattern is required.</p>
            </div>
            <div className="grid grid-cols-2 gap-2">
              <Input aria-label="Excellent minimum health" type="number" min={0} max={10}
                value={formState.dodgeConfig.performance_thresholds?.excellent_min_health ?? 3}
                onChange={(e) => updateDodgeConfig({ performance_thresholds: {
                  ...(formState.dodgeConfig.performance_thresholds ?? DEFAULT_DODGE_CONFIG.performance_thresholds!),
                  excellent_min_health: Number(e.target.value),
                } })} />
              <Input aria-label="Survive minimum health" type="number" min={0} max={10}
                value={formState.dodgeConfig.performance_thresholds?.survive_min_health ?? 1}
                onChange={(e) => updateDodgeConfig({ performance_thresholds: {
                  ...(formState.dodgeConfig.performance_thresholds ?? DEFAULT_DODGE_CONFIG.performance_thresholds!),
            ...
            {(formState.dodgeConfig.performance_thresholds?.survive_min_health ?? 1) >
              (formState.dodgeConfig.performance_thresholds?.excellent_min_health ?? 3) && (
              <p className="text-xs text-danger">Survive minimum health cannot exceed excellent minimum health.</p>
            )}
            <div className="grid grid-cols-3 gap-2">
              <Select aria-label="Player style" value={formState.dodgeConfig.player_style ?? "soul"}
                options={[{ value: "soul", label: "Soul" }, { value: "heart", label: "Heart" }, { value: "spark", label: "Spark" }]}
                onChange={(e) => updateDodgeConfig({ player_style: e.target.value as DodgeConfigShape["player_style"] })} />
              <Select aria-label="Obstacle style" value={formState.dodgeConfig.obstacle_style ?? "ash"}
                options={[{ value: "ash", label: "Ash" }, { value: "neon", label: "Neon" }, { value: "crystal", label: "Crystal" }]}
                onChange={(e) => updateDodgeConfig({ obstacle_style: e.target.value as DodgeConfigShape["obstacle_style"] })} />
              <Input aria-label="Obstacle color" type="color" value={formState.dodgeConfig.obstacle_color ?? "#ff8a65"}
                onChange={(e) => updateDodgeConfig({ obstacle_color: e.target.value })} />
            </div>
            <div className="grid grid-cols-3 gap-2">
              <Select aria-label="Arena background" value={formState.dodgeConfig.background ?? "void"}
                options={[{ value: "void", label: "Void" }, { value: "ember", label: "Ember" }, { value: "midnight", label: "Midnight" }]}
                onChange={(e) => updateDodgeConfig({ background: e.target.value as DodgeConfigShape["background"] })} />
              <Select aria-label="Arena texture" value={formState.dodgeConfig.texture ?? "none"}
                options={[{ value: "none", label: "None" }, { value: "grain", label: "Grain" }, { value: "stars", label: "Stars" }]}
                onChange={(e) => updateDodgeConfig({ texture: e.target.value as DodgeConfigShape["texture"] })} />
              <Select aria-label="Palette" value={formState.dodgeConfig.palette ?? "ashfall"}
                options={[{ value: "ashfall", label: "Ashfall" }, { value: "ember", label: "Ember" }, { value: "aurora", label: "Aurora" }]}
                onChange={(e) => updateDodgeConfig({ palette: e.target.value as DodgeConfigShape["palette"] })} />
            </div>
            <CoverImageUploader value={formState.dodgeConfig.background_asset_url ?? null}
              onChange={(background_asset_url) => updateDodgeConfig({ background_asset_url })}
              label="Custom arena background" description="Optional uploaded image only; no external URLs." />
            <div className="grid grid-cols-2 gap-2">
              <div>
                <label htmlFor="dodge-audio-upload" className="block text-xs text-content-muted">
                  Encounter audio (MP3, OGG, WAV; 10MB max)
                </label>
                <input id="dodge-audio-upload" aria-label="Upload encounter audio" type="file"
                  accept={ALLOWED_SCENARIO_AUDIO_ACCEPT} onChange={handleAudioUpload}
                ...
                {formState.dodgeConfig.audio?.music_asset_url && (
                  <p className="mt-1 text-xs text-content-faint">Audio uploaded</p>
                )}
                {audioUploadError && <p className="mt-1 text-xs text-danger">{audioUploadError}</p>}
              </div>
              <Input aria-label="Audio volume" type="number" min={0} max={1} step={0.1} value={formState.dodgeConfig.audio?.volume ?? 0.7}
                onChange={(e) => updateDodgeConfig({ audio: { ...(formState.dodgeConfig.audio ?? DEFAULT_DODGE_CONFIG.audio!), volume: Number(e.target.value) } })} />
            </div>
            <div className="grid grid-cols-2 gap-2">
              {(["instructions", "start_text", "win_text", "lose_text"] as const).map((field) => (
                <Input key={field} aria-label={`Dodge ${field.replace("_", " ")}`}
                  value={formState.dodgeConfig.copy?.[field] ?? ""}
                  onChange={(e) => updateDodgeConfig({ copy: { ...(formState.dodgeConfig.copy ?? DEFAULT_DODGE_CONFIG.copy!), [field]: e.target.value } })} />
              ))}
            </div>
            <Button type="button" variant="secondary" size="sm"
              onClick={() => setFormState({ ...formState, dodgeDifficulty: DEFAULT_DODGE_DIFFICULTY, dodgeConfig: { ...DEFAULT_DODGE_CONFIG } })}>
      ...
      {formState.outcomeMode === "tiered" && (
        <div className="space-y-2">
          ...
          {tierRangesOverlap(formState.tieredOutcomes) && (
            <p className="text-xs text-danger">
              Score ranges cannot overlap; boundary scores belong to one tier only.
            </p>
          )}
        </div>
      )}
