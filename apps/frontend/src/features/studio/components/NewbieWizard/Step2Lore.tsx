  ...
  useEffect(() => {
    const handler = setTimeout(() => {
      const changed = Object.keys(localDraft).some(
        (key) => localDraft[key as keyof typeof localDraft] !== newbieDraft[key as keyof typeof newbieDraft]
      );
      if (changed) {
        setSaveState(true);
        setTimeout(() => {
  ...
  return (
    <div className="space-y-8">
      <div className="space-y-2 border-b border-zinc-800 pb-4 flex justify-between items-start">
        <div>
          <h2 className="text-2xl font-semibold text-zinc-100 tracking-tight">World Lore</h2>
          <p className="text-sm text-zinc-400">Describe the world, characters, and history.</p>
        </div>
          
        <div className="flex items-center gap-3 bg-zinc-950 border border-zinc-800 p-2">
          <span className="text-xs font-semibold text-zinc-400 uppercase tracking-wider">Single Prompt Mode</span>
          <button 
            onClick={() => handleChange('useSingleLorePrompt', !localDraft.useSingleLorePrompt)}
          ...
          >
            <div className={`absolute top-0.5 left-0.5 w-3.5 h-3.5 bg-zinc-500 transition-transform ${localDraft.useSingleLorePrompt ? 'translate-x-5 bg-zinc-800' : 'translate-x-0'}`}></div>
          </button>
        </div>
        </div>
        ...
        {localDraft.useSingleLorePrompt ? (
          <div className="space-y-3">
            <label className="block text-sm font-semibold tracking-wide text-zinc-300 uppercase">Everything</label>
          ...
          <div className="space-y-8">
            <div className="space-y-3">
              <label className="block text-sm font-semibold tracking-wide text-zinc-300 uppercase">The World Lore</label>
              <DistractionFreeEditor 
            ...
            <div className="space-y-3">
              <label className="block text-sm font-semibold tracking-wide text-zinc-300 uppercase">Opening Prompt</label>
              <p className="text-xs text-zinc-500">How does the story begin for the player?</p>
              <DistractionFreeEditor 
            ...
            <div className="space-y-3">
              <div className="flex justify-between items-center">
                <label className="block text-sm font-semibold tracking-wide text-zinc-300 uppercase">Main Conflict / Goal</label>
                <button 
                  onClick={() => handleChange('includeConflict', !localDraft.includeConflict)}
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-center">
                <div>
                  <label className="block text-sm font-semibold tracking-wide text-zinc-300 uppercase">Story Cards</label>
                  <p className="text-xs text-zinc-500">Create separate cards for different characters, factions, or locations.</p>
                </div>
                <button
                  onClick={() => handleChange('storyCards', [...localDraft.storyCards, { id: Math.random().toString(36).substring(7), type: "Character", name: "New Card", content: "" }])}
              ...
              <div className="space-y-6">
                {localDraft.storyCards.map((card, idx) => (
                  <div key={card.id} className="bg-zinc-950 border border-zinc-800 p-4 space-y-3 relative group">
                    <button 
                      onClick={() => handleChange('storyCards', localDraft.storyCards.filter(c => c.id !== card.id))}
                    ...
                    <div className="flex gap-3 mb-2 w-full md:w-2/3">
                      ...
                      >
                        <option value="Character" className="bg-zinc-950 text-zinc-300">Character</option>
                        <option value="Faction" className="bg-zinc-950 text-zinc-300">Faction</option>
                        <option value="Location" className="bg-zinc-950 text-zinc-300">Location</option>
                        <option value="Item" className="bg-zinc-950 text-zinc-300">Item</option>
                        <option value="Other" className="bg-zinc-950 text-zinc-300">Other</option>
                      </select>
                      <input 
                        type="text" 
                        value={card.name}
