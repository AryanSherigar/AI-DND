  ...
  return (
    <ExpandablePanel summary={summary} actions={actions}>
      <div className="space-y-3 text-sm">
        {entity.description && (
          <p className="text-content-muted">{entity.description}</p>
        )}
        ...
        <div>
          <p className="text-xs font-medium text-content-muted">Attributes</p>
          {attributeEntries.length === 0 ? (
            <p className="text-xs text-content-faint">No attributes defined.</p>
          ) : (
            <table className="mt-1 w-full text-xs text-content-muted">
              <tbody>
                {attributeEntries.map(([key, schema]) => (
                  <tr key={key} className="border-t border-border-subtle">
                    <td className="py-1 pr-3 font-medium text-content-muted">
                      {schema.label ?? key}
                    </td>
                    <td className="py-1 pr-3">{schema.type}</td>
                    <td className="py-1 text-content-faint">
