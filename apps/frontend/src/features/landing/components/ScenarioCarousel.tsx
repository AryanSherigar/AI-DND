    ...
    <div className="absolute bottom-8 left-0 right-0 z-20 flex justify-center px-6">
      <div className="flex items-center gap-4 p-4 bg-white/5 backdrop-blur-xl border border-white/20 rounded-2xl shadow-[inset_0_1px_1px_rgba(255,255,255,0.2),0_8px_32px_rgba(0,0,0,0.5)]">
        ...
        <button 
          ...
          className="p-2 text-white/50 hover:text-white disabled:opacity-30 disabled:cursor-not-allowed transition-colors font-retro text-xs"
        >
          {'<'}
        </button>
          ...
          {scenarios.map((scenario, index) => {
            ...
            return (
              ...
              >
                ...
                />
                <div className={`absolute inset-0 bg-gradient-to-t from-black/80 to-transparent flex items-end p-2 transition-opacity ${isActive ? 'opacity-100' : 'opacity-0'}`}>
                  <span className="font-playfair font-medium text-sm text-white text-left leading-tight">
                    {scenario.title}
                  </span>
                </div>
              </button>
        ...
        {/* Right Arrow */}
        <button 
          onClick={() => onSelectIndex(Math.min(activeIndex + 1, scenarios.length - 1))}
