        ...
        <div className="flex-1 overflow-y-auto px-4 py-6 md:px-12 pb-24 custom-scrollbar">
          <div className="max-w-5xl space-y-2">
            <div className="text-zinc-500 font-mono text-sm border-b border-zinc-800/50 pb-4 mb-4">
              Showing results for <span className="text-zinc-300">{searchParams.get("q") || "all scenarios"}</span>
            </div>
            ...
            {isLoading ? (
              ...
              <div className="py-20 text-center flex flex-col items-center justify-center border border-dashed border-zinc-800 rounded-xl">
                 <h3 className="text-xl font-fell-sc text-zinc-400 mb-2">No adventures found</h3>
                 <p className="text-zinc-500 font-mono text-sm mb-6">Try adjusting your filters or search query.</p>
                 <button onClick={() => setSearchParams(new URLSearchParams())} className="px-4 py-2 bg-zinc-800 hover:bg-zinc-700 text-white rounded-lg font-mono text-sm transition-colors">
                   Clear all filters
                 </button>
              </div>
            )}
