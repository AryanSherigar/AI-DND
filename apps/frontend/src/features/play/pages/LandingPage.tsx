...
import { trendingScenarios, epicAdventures, newArrivals } from '../mock/scenarios';
  ...
  return (
    <AnimatePresence>
      ...
      >
        {/* Top Navbar */}
        <header
          className={`fixed top-0 w-full z-50 transition-all duration-300 ${isScrolled ? 'bg-[#0d0f14]/90 backdrop-blur-md shadow-lg shadow-black/50 py-4' : 'bg-transparent py-8'
            }`}
        >
          <div className="container mx-auto px-12 md:px-24 flex justify-between items-center">
            <div className="font-pixelify text-3xl text-white tracking-widest">
              AI-DND
            </div>
            <nav className="hidden md:flex gap-10 font-manrope font-semibold text-sm tracking-wider text-zinc-400 uppercase">
            ...
            <div className="flex items-center gap-4">
              <Link
                to="/login"
                className="flex items-center gap-2 text-zinc-300 font-pixelify text-xl hover:text-emerald-400 transition-colors"
              >
