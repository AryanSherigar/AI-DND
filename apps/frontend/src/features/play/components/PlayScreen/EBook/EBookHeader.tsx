...
export function EBookHeader({
  ...
  return (
    <div className="sticky top-0 z-30">
      <header
        className={`w-full h-14 border-b backdrop-blur-md px-4 flex items-center justify-between transition-colors ${headerStyle}`}
      >
      ...
      {activeConditions.length > 0 && (
        <div className={`border-b backdrop-blur-md ${headerStyle}`}>
          <StatusBadgeRow conditions={activeConditions} />
        </div>
      )}
    </div>
  );
