...
function drawOrbOrShard(graphics: Graphics, spawn: HazardSpawn, color: number): void {
  graphics.clear();
...
export function buildHazardLayer(arenaWidth: number, arenaHeight: number): HazardLayer {
  const container = new Container();

  const spawn = (spawns: HazardSpawn[], patternType: HazardPatternType): LiveHazard[] => {
    const color = HAZARD_COLOR_BY_PATTERN[patternType];
    return spawns.map((hazardSpawn) => {
      const graphics = new Graphics();
      renderBeamOrShape(graphics, hazardSpawn, color, arenaWidth, arenaHeight);
...
function renderBeamOrShape(
  ...
  if (spawn.shape !== "beam") {
    drawOrbOrShard(graphics, spawn, color);
    return;
  }
  const isHorizontal = spawn.x === arenaWidth / 2;
  graphics.clear();
  if (isHorizontal) {
    graphics.rect(-arenaWidth / 2, -spawn.radius, arenaWidth, spawn.radius * 2).fill(color);
    return;
  }
  graphics.rect(-spawn.radius, -arenaHeight / 2, spawn.radius * 2, arenaHeight).fill(color);
}
