...
import { HeartIcon, TextInputIcon, QuillIcon, PlayIcon } from '../../../shared/components/icons/PixelIcons';
    ...
    >
      ...
      <div className="absolute inset-0 p-5 flex flex-col justify-end z-30">
        ...
        <div className="transition-all duration-300 ease-in-out group-hover/card:-translate-y-8 group-hover/card:opacity-0 group-hover/card:invisible">
          <h3 className="font-pixelify text-2xl text-white mb-2 line-clamp-2 leading-snug drop-shadow-md">{scenario.title}</h3>
          <div className="flex items-center justify-between text-sm text-zinc-300 font-manrope font-semibold">
            <span className="flex items-center gap-1.5" style={{ color: accentColor }}>
              <HeartIcon className="w-4 h-4" /> <span className="text-zinc-300">{scenario.rating}</span>
            </span>
            <span className="flex items-center gap-1.5" style={{ color: accentColor }}>
              <TextInputIcon className="w-4 h-4" /> <span className="text-zinc-300">{scenario.playerCount}</span>
            </span>
          </div>
        </div>
        ...
        <div className="absolute bottom-0 left-0 right-0 p-5 opacity-0 translate-y-8 group-hover/card:opacity-100 group-hover/card:translate-y-0 transition-all duration-300 ease-out pointer-events-none group-hover/card:pointer-events-auto">
          <h3 className="font-pixelify text-xl text-white mb-2 line-clamp-2 leading-tight drop-shadow-md">{scenario.title}</h3>
          <p className="font-manrope text-sm text-zinc-300 line-clamp-3 mb-4 leading-relaxed">
            {scenario.logline || "No description provided."}
          </p>
          <div className="pt-3 border-t border-white/10 mt-1">
            <div className="flex justify-between items-center mb-4 font-manrope text-xs font-bold uppercase tracking-wider">
            ...
            <button 
              className="flex justify-center items-center gap-2 w-full bg-white text-zinc-950 font-pixelify text-xl py-2 hover:bg-zinc-200 transition-colors"
