...
export const AdvancedFiltersModal: React.FC<AdvancedFiltersModalProps> = ({
  ...
  const [localParams, setLocalParams] = React.useState(new URLSearchParams(searchParams));

  const handleToggleGenre = (genre: string) => {
    const genres = localParams.getAll("genre");
    if (genres.includes(genre)) {
      localParams.delete("genre");
      genres.filter(g => g !== genre).forEach(g => localParams.append("genre", g));
    } else {
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
      <div className="bg-[#0d0f14] border border-zinc-800 rounded-2xl w-full max-w-2xl max-h-[90vh] flex flex-col shadow-2xl">
        <div className="flex justify-between items-center p-6 border-b border-zinc-800">
          <h2 className="text-xl font-fell-sc font-bold text-white tracking-wider">Advanced Filters</h2>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-8 custom-scrollbar">
          {/* Complexity Tier */}
          <section>
            <h3 className="text-sm font-mono text-zinc-400 mb-3 uppercase tracking-widest">Complexity</h3>
          ...
          {/* Genres (Multi-select) */}
          <section>
            <h3 className="text-sm font-mono text-zinc-400 mb-3 uppercase tracking-widest">Genres</h3>
            <div className="flex flex-wrap gap-2">
              {GENRES.map(genre => {
                const isActive = localParams.getAll("genre").includes(genre.toLowerCase());
                return (
                  <button
                    key={genre}
           ...
           {/* Minimum Plays */}
           <section>
            <h3 className="text-sm font-mono text-zinc-400 mb-3 uppercase tracking-widest">Minimum Plays</h3>
            <select
        ...
        <div className="p-6 border-t border-zinc-800 flex justify-between items-center bg-zinc-950/50">
          <button onClick={handleClear} className="text-sm font-mono text-zinc-500 hover:text-zinc-300">
            Clear all
          </button>
          <div className="flex gap-3">
             <button onClick={onClose} className="px-5 py-2 rounded-lg font-mono text-sm border border-zinc-800 text-zinc-300 hover:bg-zinc-800">
               Cancel
             </button>
             <button onClick={handleApply} className="px-5 py-2 rounded-lg font-mono text-sm font-bold bg-white text-black hover:bg-zinc-200 shadow-[0_0_15px_rgba(255,255,255,0.2)]">
               Show Results
             </button>
          </div>
        </div>
