"""Scenario ORM model (mirrors core-api's schema).

TRS may only ever mutate `play_count` on this table (CLAUDE.md); see
app/repositories/scenario_repo.py, which enforces that at the repository
boundary by exposing no other write method.
"""
