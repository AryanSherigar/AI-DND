"""Structlog configuration for core-api.

Intentionally duplicated (not shared) with the identically-shaped
apps/turn-resolution-service/app/logging_config.py — no shared Python
package exists between the two services, and this file is small and
stable enough that duplication beats introducing one for it. Keep both
copies in sync by hand. See docs/logging.md for the policy rationale
behind the redaction denylist and event-category split.
"""
